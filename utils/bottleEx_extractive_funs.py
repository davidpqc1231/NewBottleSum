...
############
def elimination_beam_search(S1, S2, S3, # input sentences
############
                            k = 1, 
                            rem_words = 1, 
                            tok_method = 'moses', 
                            ...
                            model_path = '',
                            #################
                            max_tokens_batch = 2000, # based on GPU memory, for efficiency
                            #################
                            model = None,
                            tokenizer = None,
    ...
    ####################
    S1_score, S2_score, S3_score = get_CE_scores([S1], S2, S3, tokenizer, model, max_tokens_batch) #, tok_method = tok_method)
    ####################
    score_S1_og = S1_score[0] # need these for the filter function later
    score_S2_og = S2_score[0]
    score_S3_og = S3_score[0]
    og = {'S1_':S1 , 'split_S1_': S1split,
          'CE_S1_': score_S1_og,'CE_S2':score_S2_og,'CE_S3':score_S3_og,
    ...
    ########################
    score_beam = lambda v: v['CE_S2'] + v['CE_S3']
    ########################
    ...
    for i in range(len_S1, min_words, -1): # populate S1_list with sentences of length 
        ...
        S1_scores, S2_scores, S3_scores = get_CE_scores(candidates, S2, S3, tokenizer, model, max_tokens_batch) #, tok_method = tok_method) 
        ...
        for j, candidate_split in enumerate(candidates_split):
            ...
            candidate_dict = {'S1_': candidate, 'split_S1_':candidate_split,
                              'CE_S2':S2_scores[j], 'CE_S3':S3_scores[j], 'CE_S1_':S1_scores[j],
