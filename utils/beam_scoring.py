...
##############
def get_CE_scores(S1_list, S2, S3, tokenizer, model, max_tokens_batch):
##############
    '''
    This funciton takes:
    ...
    ###################
    S3 = tokenizer.encode(S3)
    S3_list = [S3]*len(S1_list)
    ###################
    ...
    ###################
    S3_scores = get_CE_list(model, [tokenizer.encode(' ') + S1 for S1 in S1_list], S3_list, batch=batch_size, red = True)
    ###################
    ...
    return S1_scores, S2_scores, S3_scores
